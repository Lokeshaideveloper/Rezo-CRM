    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl">
        ...
        <div className="p-5 space-y-4">
          ...
          {/* Subject */}
          <div className="flex items-center gap-3 border border-slate-200 rounded-lg px-3 py-2.5 focus-within:border-indigo-400 transition-colors">
          ...
          {/* Body */}
          <textarea
            className="w-full border border-slate-200 rounded-lg px-4 py-3 text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:border-indigo-400 transition-colors resize-none min-h-[200px] leading-relaxed"
...
export default function DealDetailPage() {
  ...
  if (!deal) return (
    <div className="p-8 text-center text-slate-500">
      Deal not found. <Link href="/deals" className="text-indigo-600 underline">Back to deals</Link>
    </div>
  )
    ...
    <div className="min-h-screen bg-slate-50">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-start gap-4">
            <div className="text-right">
              <p className="text-2xl font-bold text-indigo-600">{formatCurrency(deal.expected_mrr)}</p>
      ...
      <div className="p-8 max-w-4xl">
            ...
            >
              ...
              {count > 0 && (
                <span className={`text-xs px-1.5 py-0.5 rounded-full font-bold ${
                  activeTab === key ? 'bg-indigo-500 text-white' : 'bg-slate-200 text-slate-600'
                }`}>{count}</span>
              )}
            </button>
          ))}
        ...
        {activeTab === 'attachments' && (
          <div className="space-y-4">
            <div
              className="bg-white rounded-xl border-2 border-dashed border-slate-300 hover:border-indigo-400 transition-colors p-8 text-center cursor-pointer group"
            ...
            >
              ...
              {uploadingFile ? (
                ...
                <div className="flex flex-col items-center gap-2">
                  ...
                  <p className="text-sm font-semibold text-slate-600 group-hover:text-indigo-600 transition-colors">Click or drag a file to upload</p>
                  ...
                  {attachments.map(att => (
                    <div key={att.id} className="flex items-center gap-4 px-5 py-3.5 hover:bg-slate-50 transition-colors group">
                      ...
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => handleDownload(att)} className="p-1.5 hover:bg-indigo-50 rounded-lg text-slate-400 hover:text-indigo-600 transition-colors">
        ...
        {activeTab === 'emails' && (
          <div className="space-y-4">
            ...
            <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-4 flex items-center justify-between">
              <div>
                <p className="text-sm font-semibold text-indigo-800">Send an email to {contactName || 'contact'}</p>
                <p className="text-xs text-indigo-500 mt-0.5">{contactEmail || 'No contact email set on this deal'}</p>
              </div>
              <button
                onClick={() => setShowEmailModal(true)}
